...
/////////////////////////////////////////////////////////////////////////////////////
//
//	BaseIRForward (6150's Primary Autonomous Program)
//
...
//	needs to be setup with the light sensor in line with the line in front of the baskets.
//	Our robot first finds the IR beacon and drops one autonomous block into
//	the corresponding basket, which consistently scores. The second part of autonomous
//	drives our robot onto the ramp with enough space for another alliance member.
// 	In all, we can score 60 points in autonomous consistently and without flaws.
//
// 	Last modified: Jan 11th, 2014
//
//	DOCUMENTATION AND CHANGES:
//	12/31/13 - Added this comment and other to explain certain parts of the code.
//	1/11/14 - Made slight adjustments in turns and drops to the basket. Works as of 3:30 pm
//
...
int lineUp()
{
	...
	while(SensorValue(IR) != searchZone) // 6 for other side
	{
		if(SensorValue(IR) < searchZone) // if IR sensor is less than the searchZone ...
		{
			...
			iterations--; // decrement the times searched
		}
	...
	while(SensorValue(IR) != 4) // always 4
	{
		motor[motorD] = -50;
		motor[motorE] = 50;
	}
...
void ramp(int iterations)
{
	...
	wait1Msec(100 * (iterations - 2));// iterations normally 2
...
task main()
{
	...
	ramp(timesRun); // see 172 for ramp(timeRuns)
}
